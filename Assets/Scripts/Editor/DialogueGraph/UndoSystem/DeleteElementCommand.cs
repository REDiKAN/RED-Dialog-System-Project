...
public class DeleteElementCommand : GraphCommand
{
    ...
    private string guid;
    private string serializedNodeData;
        ...
        : base(graphView)
    {
        ...
        if (element is BaseNode node)
        {
            ...
            // Сериализуем данные узла перед удалением
            serializedNodeData = node.SerializeNodeData();
        }
    }
    ...
    public override void Execute()
    {
        if (element is BaseNode node)
        {
            ...
            // Сохраняем данные для восстановления связей
    ...
    public override void Undo()
    {
        if (elementType != null && typeof(BaseNode).IsAssignableFrom(elementType))
        {
            ...
            if (!string.IsNullOrEmpty(serializedNodeData))
            {
                node.DeserializeNodeData(serializedNodeData);
            }
    ...
    private Port FindPortByName(BaseNode node, string portName)
    {
        return node.outputContainer.Children().OfType<Port>().FirstOrDefault(p => p.portName == portName) ??
               node.outputContainer.Children().OfType<Port>().FirstOrDefault();
    }
}